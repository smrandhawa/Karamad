...
$r= getResult($worker_id,$carrier,$carrier_entered);
	echo $r;

mysql_close($db);	// Close DB connection

function getResult($worker_id,$carrier,$carrier_entered)
{
	// Search if it exists.
	...
	if ($result){
		if(mysql_num_rows($result) != 0){	// Does not exist. Okay, Insert.
			$query="UPDATE  `smartsurveys`.`worker` SET `carrier`= '$carrier', `carrier_entered`= $carrier_entered WHERE `worker_id` = $worker_id";
