...
function getResult($worker_id)
{
	...
	//echo json_encode($availables_surveys_ids_filtered_by_question_conditions);
	...
	//echo json_encode($surveys_ids_filtered_by_responses);
	$surveys_ids = join(",",$surveys_ids_filtered_by_responses);
	$rows = array();
	if($surveys_ids)
	{
		$query = "SELECT * from `smartsurveys`.`survey` where `survey`.`survey_id` IN ($surveys_ids) ORDER BY `survey`.`time_modified` DESC";
