...
{	
	...
	if ($result){
		...
		if($count==0){
			$query = " INSERT INTO `Baang`.`uservotesofrec` (`recid`,`userid`,`type`) Values ($id,$userid,2) ";
			$result = mysql_query($query);
		
			$query = "UPDATE `Baang`.`recording_table` set downvotes = downvotes + 1 WHERE Recording_id= $id  ";
			$result = mysql_query($query);
